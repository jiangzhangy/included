...
<a href="<?php echo U(MODULE_NAME.'/Advertise/add');?>" class="btn btn-success"><i class="icon-plus icon-white"></i> 添加广告</a>
</div>
	...
	<table class="table table-bordered table-wordpress postlist-table">
		...
		<tbody>
		    <?php if(is_array($list)): foreach($list as $key=>$list): ?><tr class="tr-publish">
                ...
                <td><img src="<?php echo $list['img_url'];?>" style="width:60px; height:30px;overflow:hidden;"/></td>
				<td><?php echo $list['title'];?></td>
				<td><?php echo $list['site_url'];?></td>
                <td><?php echo getAdvName($list['cate_id']);?></td>
				...
				<td>
					<div class="btn-toolbar">
						<div class="btn-group">
							...
							<a href="javascript:void(0);" class="btn btn-mini btn-danger" onclick="return confirm('确定删除此图片吗？',function(){del(<?php echo $list['id'] ?>)});"><i class="icon-remove"></i> 删除</a>
						</div>
					</div>
				</td>
