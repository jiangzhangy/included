...
<a href="<?php echo U(MODULE_NAME.'/'.$controller_name.'/add');?>" class="btn btn-success"><i class="icon-plus icon-white"></i> 添加页面</a>
</div>
<h4><i class="icon-home"></i>页面管理</h4>
<hr/>
	<table class="table table-bordered table-wordpress postlist-table">
		...
		<tbody>
		    <?php if(is_array($list)): foreach($list as $key=>$list): ?><tr class="tr-publish">
				...
				<td><a target="_blank" href="<?php echo U('Home/Index/Page',array('id'=>$list['id']));?>"><?php echo $list['title'];?></a></td>
