...
<a href="<?php echo U(MODULE_NAME.'/'.$controller_name.'/add');?>" class="btn btn-success"><i class="icon-plus icon-white"></i>新增导航</a>
</div>
	...
	<table class="table table-bordered table-wordpress postlist-table">
		...
		<tbody>
		    <?php if(is_array($list)): foreach($list as $key=>$list): ?><tr class="tr-publish">
				...
				<td><a target="_blank" href="<?php echo $list['url'];?>"><?php echo $list['name'];?></a></td>
				<td> <?php if($list['open_type'] == '_brank'): ?>新窗口<?php else: ?>当前窗口<?php endif; ?></td>
               
               <?php if($list['id'] != 1): ?><td onclick="status(<?php echo $list['id'];?>)" id="status_<?php echo $list['id'];?>">
               ...
               <span style="cursor:pointer;" class="btn disabled btn-mini"><i class="icon-ban-circle"></i></span><?php endif; ?>
				...
				<td>
					<div class="btn-toolbar">
						<div class="btn-group">
							<?php if($list['id'] != 1): ?><a href="<?php echo U(MODULE_NAME.'/'.$controller_name.'/edit',array('id'=>$list['id'],'p'=>$_GET['p']));?>" class="btn btn-mini" ><i class="icon-edit"></i> 更改</a>
							 <a href="javascript:void(0);" class="btn btn-mini btn-danger" onclick="return confirm('确定删除此记录吗？',function(){del(<?php echo $list['id'] ?>)});"><i class="icon-remove"></i> 删除</a><?php endif; ?>
						</div>
					</div>
				</td>
