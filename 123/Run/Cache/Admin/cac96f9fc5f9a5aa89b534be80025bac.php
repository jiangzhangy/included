...
<div class="main span10">
    <h4><i class="icon-edit"></i>编辑文章<a href="<?php echo U(MODULE_NAME.'/'.$controller_name.'/index');?>" class="pull-right" ><i class="icon-backward"></i> <strong>返回列表</strong></a></h4>
<hr>
        ...
        <td><input type="text" name="name" value="<?php echo $vo['name'];?>" id="name" class="span4"  />                </td>
      </tr>  
      <tr><td>
        <label for="title">模块</label></td>
        <td><select name="moudle">
          <option value="">请选择</option>
          <option value="news" <?php if($vo['moudle'] == 'news'): ?>selected="selected"<?php endif; ?>>文章</option>
          <option value="product" <?php if($vo['moudle'] == 'product'): ?>selected="selected"<?php endif; ?>>产品</option>
          <option value="page" <?php if($vo['moudle'] == 'page'): ?>selected="selected"<?php endif; ?>>页面</option>
        </select>
      </td>
      </tr> 
       <tr id="moudle_id"><td>
        ...
        <td>
          <select name="moudle_id">
           <?php if(is_array($moudle)): foreach($moudle as $key=>$moudle): ?><option value="<?php echo $moudle['id'];?>" <?php if($vo['moudle_id'] == $moudle['id']): ?>selected="selected"<?php endif; ?>><?php if($moudle['level'] > 1): echo $moudle['name']; endif; echo $moudle['subject'];?></option><?php endforeach; endif; ?>
         </select>
      </td>
      </tr>
       <tr><td>
        ...
        <option value="_brank" <?php if($vo['open_type'] == '_brank'): ?>selected="selected"<?php endif; ?>>新窗口</option>
        <option value="_parent" <?php if($vo['open_type'] == '_parent'): ?>selected="selected"<?php endif; ?>>当前窗口</option>
        </select>
        </td>
      </tr>           
      <tr><td>
...
$(function(){
 //获取城市
  $(document).on('change','select[name="moudle"]',function(){
    var url = "<?php echo U(MODULE_NAME.'/'.$controller_name.'/get_module');?>";
    if($(this).val()!=''){
