...
<div class="main span10">
    <h4><i class="icon-plus"></i>添加栏目<a href="<?php echo U(MODULE_NAME.'/'.$controller_name.'/index');?>" class="pull-right" ><i class="icon-backward"></i> <strong>返回栏目列表</strong></a></h4>
<hr>
		<form action="<?php echo U(MODULE_NAME.'/'.$controller_name.'/insert');?>" method="post" accept-charset="utf-8" class="well form-inline" id="f" onSubmit="return ajaxform(this,function(d){opt_ok_return(d)})">                		
            ...
            <?php if(is_array($cate)): foreach($cate as $key=>$cate): ?><option value="<?php echo $cate['id'];?>"><?php if($cate['level'] > 1): echo $cate['name']; endif; echo $cate['subject'];?></option><?php endforeach; endif; ?>
            </select>
             </td>
            </tr>
            <tr><td>
				<label for="subject">栏目名称</label></td>
				<td><input type="text" name="subject" value="" id="subject" class="span4"  />                </td>
			</tr>
            <tr><td>
				<label for="sort">排序</label></td>
				<td><input type="text" name="sort" value="0" id="sort" class="span4"  /></td>
			</tr>
            <tr><td></td><td>
			<input type="submit" name="submit" value="添加" id="submit" class="btn btn-large btn-success"  />        </td></tr>
        </tbody>
        </table>
